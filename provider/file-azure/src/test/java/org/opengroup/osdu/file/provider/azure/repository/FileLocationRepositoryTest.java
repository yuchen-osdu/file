...
class FileLocationRepositoryTest {
  ...
  private static final String AZURE_LOCATION = "https://storage.blob.windows.net/container/file.tmp";
  ...
  void setUp() {
    fileLocationRepository
        = new FileLocationRepository(fileLocationMapper, fileLocationEntityRepository);
  }
  ...
  void shouldFindFileLocationEntityByFileId() {
    ...
    String entityId = UUID.randomUUID().toString(); // RandomUtils.nextInt();
  ...
  private FileLocationEntity getFileLocationEntity(String id, Date createdDate) {
